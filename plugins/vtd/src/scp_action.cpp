...
cloe::TriggerSchema ScpActionFactory::schema() const {
  ...
  return TriggerSchema{
    ...
    schema::Variant{
      Schema{
        {"text", make_prototype<std::string>("raw SCP text to send").require()}
      },
      Schema{
        {"ref", make_prototype<std::string>("use predefined template").require()},
...
cloe::ActionPtr ScpActionFactory::make(const cloe::Conf& c) const {
  if (c.has("text")) {
    auto text = c.get<std::string>("text");
    return std::make_unique<ScpAction>(name(), scp_client_, text);
  } else {
    auto key = c.get<std::string>("ref");
    ...
    return std::make_unique<ScpAction>(name(), scp_client_, tmpl);
  }
}
